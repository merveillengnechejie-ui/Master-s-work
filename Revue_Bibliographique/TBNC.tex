...
%TODO ajouter une introduction avec annonce du plan
...
Récemment, le concept de \textbf{Portes Logiques Moléculaires (Molecular Logic Gates)} activées par l'hypoxie a émergé comme une stratégie de rupture \cite{Wang2026, Liu2025}. Dans ce modèle, l'enzyme \textbf{Nitroreductase (NTR)}, surexprimée dans les zones hypoxiques des tumeurs TNBC, agit comme une "clé" enzymatique. Elle transforme un précurseur nitro-BODIPY ("OFF", non-fluorescent, faible PDT) en un agent amino-BODIPY ("ON", fluorescent, forte PDT/PTT). Cette activation sélective permet de minimiser la toxicité systémique tout en maximisant l'effet thérapeutique au cœur de la tumeur.
...
\end{itemize}
\section*{Conclusion}
Le TNBC, par son absence de cibles conventionnelles, son agressivité et ses vulnérabilités métaboliques bien caractérisées, représente un modèle idéal pour développer des agents théranostiques innovants basés sur le concept de \textbf{Porte Logique Moléculaire}. La combinaison d'imagerie par fluorescence et de thérapie photodynamique/photothermique n'est plus statique, mais \textbf{activable} par le microenvironnement tumoral (hypoxie/NTR). La conception de BODIPY modifiés (iodation positions 2,6, substitution 3,5 par donneurs $\pi$, greffage de vecteurs mitochondriaux cationiques) vise précisément à transformer ces faiblesses en opportunités thérapeutiques sélectives, où la transformation chimique induite par la NTR déclenche la synergie théranostique attendue \cite{Zhang2022, Wang2025c, Wang2026, Lu2025, Overchuk2023, Li2025}.